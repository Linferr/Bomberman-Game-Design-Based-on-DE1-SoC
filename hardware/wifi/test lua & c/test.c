...
int main() {
    ...
    // write data to RS232 UART core
    //char* str = "dofile(\"esp_wifi.lua\")";
    char str[] = "dofile(\"led.lua\")";
    for (int i = 0; str[i] != '\0'; i++){
        alt_up_rs232_write_data(rs232_dev,str[i]);
        printf("%c", str[i]);
    }
    //printf("finish write");
...
void send_code(alt_up_rs232_dev* rs232_dev, char * command);
